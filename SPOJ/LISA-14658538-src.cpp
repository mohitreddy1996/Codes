...
int num[105],ops[105];
char ch[105];
int maxdp[105][105],mindp[105][105];
int solvemax(int i,int j)
{
    ...
    if(maxdp[i][j]==-1)
    {
        ...
        for(int k=i;k<j;k++)
        {
            int temp= ops[k]=='*'?solvemax(i,k)*solvemax(k+1,j):solvemax(i,k)+solvemax(k+1,j);
            maxdp[i][j]=max(maxdp[i][j],temp);
        }
        
    }
    return maxdp[i][j];
}
int minsolve(int i,int j)
{
    if(i==j)
    {
        return num[j];
    ...
    if(mindp[i][j]==-1)
    {
        ...
        for(int k=i;k<j;k++)
        {
            int temp= ops[k]=='*'?minsolve(i,k)*minsolve(k+1,j):minsolve(i,k)+minsolve(k+1,j);
            mindp[i][j]=min(mindp[i][j],temp);
        }
    }
    return mindp[i][j];
}
int main()
{
    ...
    while(t--)
    {
        ...
        int max1=solvemax(0,l-1);
        int min1=minsolve(0,l-1);
        printf("%d %d\n",max1,min1);
